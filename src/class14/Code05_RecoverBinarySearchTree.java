...
//		TreeNode[] errors = error(root);
//		int tmp = errors[0].val;
//		errors[0].val = errors[1].val;
//		errors[1].val = tmp;
//	}
//
//	public TreeNode[] error(TreeNode root) {
...
//		while (cur != null) {
//			mostright = cur.left;
//			if (mostright != null) {// 如果无左树 则cur已处于当前子树相对最左位置 可以直接和pre比较 如果有 通过找左树最右来确定pre和cur并比较
//				while (mostright.right != null && mostright.right != cur) {// 找到原左树最右节点
//					mostright = mostright.right;
//				}
//				if (mostright.right == null) {// 如果未上连 连向cur
//					mostright.right = cur;
//					cur = cur.left;// cur向左 直到到最左 即中序的第一个数
//					continue;
//				} else {
//					mostright.right = null;// 上连已经发挥完找中序下一个位置的作用 取消连线 接下来比较pre和cur
//				}
//			}
//			if (pre != null && pre.val >= cur.val) {// 比较中序结果中相邻两位置的大小
//				e1 = e1 == null ? pre : e1;// 如果没有得到过结果 记录下来 否则不变
//				e2 = cur;// 每次都更新结果以保证记录了最后一次出现降序的第二个节点
//			}
//			pre = cur;
//			cur = cur.right;// 直接找到下一个cur/cur处于某左树最右位置 通过上连的辅助 正确跳向中序的下一个cur
//		}
//		ans[0] = e1;
//		ans[1] = e2;
//		return ans;
