...
//		int n = arr.length;
//		int[] left = new int[n];// 以i结尾范围内最大累加和子数组开始位置
//		// left[k - 1] = 0;
//		int[] knum = new int[n];// 以i开头的k个数的累加和
//		int lastk = 0;
//		for (int i = 0; i < k; i++) {
//			lastk += arr[i];// 前k个数
//		}
//		knum[0] = lastk;
//		for (int i = 1; i <= n - k; i++) {
//			knum[i] = knum[i - 1] - arr[i - 1] + arr[i + k - 1];
//		} // 填knum
//		for (int end = k; end <= n - 2 * k - 1; end++) {// 遍历首个数组的可能的结尾位置
//			lastk = knum[end - k + 1];
//			left[end] = lastk > knum[left[end - 1]] ? end - k + 1 : left[end - 1];
//		}
//		int[] right = new int[n];// 以i开头范围内最大累加和子数组开始位置
//		right[n - k] = n - k;
//		for (int R = n - k - 1; R >= 2 * k; R--) {
//			lastk = knum[R];
//			right[R] = lastk < knum[right[R + 1]] ? right[R + 1] : R;
//		}
//		int max = 0;
//		int a = 0;
//		int b = 0;
...
//			int p1 = knum[left[L - 1]];
//			int p2 = knum[L];
//			int p3 = knum[right[L + k]];
//			if (p1 + p2 + p3 > max) {
//				max = p1 + p2 + p3;
//				a = left[L - 1];
//				b = L;
